...
Para consumi-la, a arquitetura exige o uso de chaves para desestruturação do objeto, permitindo a aplicação de um apelido semântico (*Alias*) via palavra-chave `as`:
...
Em engenharia de software, um *Dry Run* representa uma execução experimental seca. O motor do `node-pg-migrate` se conecta ao PostgreSQL, inspeciona a tabela interna de controle `pgmigrations`, calcula quais arquivos na pasta `infra/migrations` ainda não foram aplicados e monta o JSON descritivo. 
