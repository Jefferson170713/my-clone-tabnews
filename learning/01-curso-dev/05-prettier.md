...
O **Prettier** é um formatador de código opinativo (*opinionated code formatter*). Ele serve para automatizar completamente a estética do código, garantindo que todo o projeto siga exatamente o mesmo padrão visual (estilo de aspas, pontos e vírgulas, quebras de linha e espaçamentos).
...
*(O comando também pode ser executado como `npm install --save-dev prettier`)*

### O Impacto Arquitetural no `package.json`
A flag `-D` avisa ao NPM que o Prettier é uma **dependência de desenvolvimento** (*devDependencies*). 
