...
Este documento consolida os aprendizados da aula 15 do curso.dev, focando na padronização de formatação de código em ambientes multi-editor e multi-plataforma através do arquivo `.editorconfig`.
...
- **`[*]`**: Um seletor global utilizando *wildcard* (caractere curinga). Significa que as regras declaradas logo abaixo deste bloco devem ser aplicadas a **todos os arquivos** de qualquer extensão dentro do projeto (ex: `.js`, `.json`, `.md`, `.css`).
