...
3. **Exportação Padrão (`export default`):** O Next.js exige que cada arquivo dentro da pasta `pages/` possua uma exportação padrão. Isso diz ao framework: *"Quando o usuário acessar esta rota, pegue este componente específico e renderize-o na tela"*.